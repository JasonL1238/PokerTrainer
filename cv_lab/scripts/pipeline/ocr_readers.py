...
class AmountRead:
    ...
    digits: int
    # The measured native glyph-run height in pixels, attached to every refusal
    # issued after the run was measured (None before P3 finds a run). This is
    # what makes the renormalized re-read DETERMINISTIC: the one resize factor
    # that puts these glyphs at the template canvas height is derived from it,
...
class TemplateOCR:
    ...
    def read_number_detail(
        ...
        There are no waiver flags. The multi-scale recovery stack that needed
        them (``skip_unexplained_ink`` to bypass P2 under consensus,
        ``allow_b8_suffix`` to admit B/8 terminator confusions) is deleted:
        below-band crops are renormalized to the template scale and re-read
        under this same full contract instead (see _renormalized_reread).
        ...
        if run_h < _MIN_CALIBRATED_RUN_H:
            return AmountRead(None, digits, score, "below_calibrated_render_size", n, run_h=run_h)
        if run_h > _MAX_CALIBRATED_RUN_H:
            return AmountRead(None, digits, score, "above_calibrated_render_size", n, run_h=run_h)
        ...
        if (x0 <= 0 or x1 >= crop_w - 1 or run_y0 <= 0 or run_y1 >= crop_h - 1
            ...
            return AmountRead(None, digits, score, "run_clipped", n, run_h=run_h)
        ...
        if _run_is_truncated(run, policed, dots, named, sep_ids):
            return AmountRead(None, digits, score, "unexplained_ink_in_numeral", n, run_h=run_h)
        ...
        if term_labels != ["B", "B"]:
            return AmountRead(None, digits, score, "suffix_not_bb", n, run_h=run_h)
        ...
        if len(sep_dots) > 1:
            ...
            return AmountRead(None, digits, score, "separator_unreconciled", n, run_h=run_h)
        if sep_dots:
        ...
        if split_at is None and widest_hole >= _DECIMAL_BAND_MIN_GAP:
            ...
            return AmountRead(None, digits, score, "integer_over_decimal_band", n, run_h=run_h)
        if split_at is not None and widest_hole > _INTRA_NUMERAL_MAX_GAP:
            ...
            return AmountRead(None, digits, score, "unexplained_gap_in_numeral", n, run_h=run_h)
        ...
        if n > 1 and digits[0] == "0" and split_at != 1:
            return AmountRead(None, digits, score, "leading_zero_no_dot", n, run_h=run_h)

        raw = f"{digits[:split_at]}.{digits[split_at:]}" if split_at is not None else digits
        try:
            value = float(raw)
        except ValueError:                                   # pragma: no cover
            return AmountRead(None, raw, score, "no_digit_run", n, run_h=run_h)
...
# The renormalized re-read's one target: put the measured glyph run at the
# digit-template canvas height, where the bank is calibrated by construction.
_RENORM_TARGET_H = float(DIGIT_SIZE[0])
# Factor bounds, both derived: below the lower bound the native run was already
# above the calibrated ceiling by more than the canvas can absorb; above the
# upper bound the native glyphs were under the segmentation floor
# (_GLYPH_MIN_H_PX) and an upscale would be enlarging noise, not text.
_RENORM_MIN_FACTOR = _RENORM_TARGET_H / float(_MAX_CALIBRATED_RUN_H)
_RENORM_MAX_FACTOR = _RENORM_TARGET_H / float(_GLYPH_MIN_H_PX)
# Never re-read from these: a crop with no digit run at all is a sprite
# fragment or empty felt, and any resize that conjures digits out of it is
# inventing a value (the measured 0.0-from-a-chip-sprite failure mode).
_RENORM_EXCLUDED = frozenset({"no_digit_run"})


def _renormalized_reread(
    bank: TemplateOCR, crop: np.ndarray, native: AmountRead,
    anchor_scale: float | None,
) -> AmountRead:
    """Deterministic re-read of a refused crop at DERIVED scale, never searched.

    At most two candidate factors, each with its own derivation:
      * 1 / anchor_scale -- the table anchor's fitted scale, mapping the crop
        back to the reference render the template bank was calibrated on;
      * canvas / run_h  -- the measured native glyph-run height mapped straight
        to the digit-template canvas height.
    Each re-read runs the FULL strict contract -- every predicate, the
    two-sided calibrated band included, now checked at the normalized scale.
    A value is accepted only from that contract, and only when the derived
    scales do not contradict each other: two successes with different values
    refuse. No waivers, no factor search, no per-size tuning -- the deleted
    recovery stack's 18 hand-picked factors, ink waivers and fraction-parsing
    heuristics recovered two more of the six pinned job-4 crops than this
    does, and that margin is exactly the tuned-to-one-recording complexity
    this replaces (the two crops now refuse with a named code; they never
    read wrong). On failure the ORIGINAL refusal is returned, so the
    diagnostic channel still names what the native read saw.
    """
    native_digits = _digits_only(native.raw or "")
    if native_digits and set(native_digits) <= {"0"}:
        # Lone under-floor zeros are the measured sprite-fragment / lost-
        # leading-decimal failure modes; true 0 BB stacks read natively.
        return native
    factors: list[float] = []
    if anchor_scale and anchor_scale > 0:
        factors.append(1.0 / float(anchor_scale))
    if native.run_h:
        factors.append(_RENORM_TARGET_H / float(native.run_h))
    readings: list[AmountRead] = []
    tried: list[float] = []
    for factor in factors:
        if not (_RENORM_MIN_FACTOR <= factor <= _RENORM_MAX_FACTOR):
            continue
        if abs(factor - 1.0) < 0.02:
            continue             # the same pixels would be the same read
        if any(abs(factor - f) < 0.02 * f for f in tried):
            continue             # both derivations agree on the scale already
        tried.append(factor)
        interpolation = cv2.INTER_CUBIC if factor > 1.0 else cv2.INTER_AREA
        resized = cv2.resize(crop, None, fx=factor, fy=factor,
                             interpolation=interpolation)
        retried = bank.read_number_detail(resized)
        if retried.value is not None:
            readings.append(retried)
    if not readings:
        return native
    if len({round(r.value, 2) for r in readings if r.value is not None}) > 1:
        return native            # the derived scales contradict each other
    return readings[0]


def read_amount_detail_from_image(
    img_bgr: np.ndarray, xyxy: Sequence[float], *,
    ...
    ``anchor_scale`` is the frame's fitted table scale relative to the
    reference render (TableAnchor.s). It is only a fallback hint for the
    renormalized re-read when the native refusal measured no glyph run."""
    bank = _bank()
    if bank is None:
        return None
    ...
    if detail.value is not None or crop.size == 0:
        return detail
    if detail.decimal_source in _RENORM_EXCLUDED:
        return detail
    return _renormalized_reread(bank, crop, detail, anchor_scale)
