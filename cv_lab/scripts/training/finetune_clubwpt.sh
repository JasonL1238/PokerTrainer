...
#
#   bash cv_lab/scripts/training/finetune_clubwpt.sh path/to/yolo_cards_clubwpt_v1
#
...
python cv_lab/scripts/training/build_mixed_card_dataset.py \
  --clubwpt "$CLUBWPT_DIR" \
  --out "$MIXED_DIR" \
  --rehearsal-frac 0.25 \
  --min-per-class 8
...
python cv_lab/scripts/training/train_yolov12_cards.py \
  --weights "$OLD_WEIGHTS" \
  --data "$MIXED_YAML" \
  --epochs 60 \
  --name "$RUN_NAME" \
